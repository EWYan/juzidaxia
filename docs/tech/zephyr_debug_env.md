...
### 可用脚本列表：
- `download-deps.sh` - 下载系统依赖包
- `install-deps-offline.sh` - 离线安装依赖
- `download-sdk.sh` - 下载Zephyr SDK
- `install-sdk.sh` - 安装SDK
- `download-zephyr-src.sh` - 下载Zephyr源码
- `download-python-deps.sh` - 下载Python依赖
- `setup-offline.sh` - 完整的离线安装脚本
- `my-first-app-build.sh` - 示例应用构建脚本
...
# 下载所有脚本（或从本文档的scripts目录复制）
# 脚本已包含在本文档的附件中，路径为：/scripts/
```
